...
import limiterwifi.networking.utils as netutils
from .menu import CommandMenu
from limiterwifi.console.io import IO
from limiterwifi.console.benner import get_main_bener
from limiterwifi.networking.host import host
from limiterwifi.networking.limiter import Limiter
from limiterwifi.networking.spoof import ARPSpoofer
from limiterwifi.networking.scan import HostScanner


class MainMenu(CommandMenu):
    def __init__(self, version, interface, gateway_ip, gatway_mac, netmask):
        super().__init__()
        self.prompt = '({}Main{}} >>>'.format(IO.Style.BRIGHT, IO.Style.RESET_ALL)
        self.parser.add_subparser('scan', self._scan_hadler)
